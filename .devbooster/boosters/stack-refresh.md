...
- Preserve the normal Stack Refresh methodology and artifact rules.
- Respect the handoff's `max_stage`, `allow_major_updates`, `allow_risky_changes`, and `return_after` fields.
- For a safe dependency wave, never advance beyond Stage 2. Do not apply major, migration, ecosystem-coupled, or otherwise risky upgrades.
- Stop and return control to Intel when the authorized wave completes, a blocker is found, a new approval is required, or the maximum stage is reached.
- Update the Stack Refresh artifact before returning and include the Intel artifact reference.
...
This mode is opt-in and must not change the behavior of a direct manual `@StackRefresh` activation.

...
> 💡 **Script paths are relative to `.zed/` directory**

#### 1. Run All Verifications (RECOMMENDED)

```bash
# SINGLE COMMAND - Runs all checks in priority order:
python .zed/scripts/verify_all.py . --url http://localhost:3000
...
# P0: Security Scan
python .zed/scripts/security_scan.py .

# P1: UX Audit
python .zed/scripts/ux_audit.py .

# P3: Lighthouse (requires running server)
python .zed/scripts/lighthouse_audit.py http://localhost:3000

# P4: Playwright E2E (requires running server)
python .zed/scripts/playwright_runner.py http://localhost:3000 --screenshot
```

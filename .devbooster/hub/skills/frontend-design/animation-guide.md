...
| **Fast ease-out** | Playful UI        | Energetic but grounded — **no bounce/elastic** (AI tell) |
...
| **Image**          | Lift/overlay; zoom only when "view closer" is the real intent | "View closer" (don't zoom every image — AI tell) |
...
### AI-Motion Tells (detected patterns — avoid)

- ❌ **Bounce/elastic easing** on generic UI — toy-like "fun" default (see Easing §2)
- ❌ **Pulsing dot** as a fake "live" signal — use only for real real-time state
- ❌ **Marquee** — endless scrolling text no one can read
- ❌ **Decorative blinking cursor** — fake typing/terminal effect
- ❌ **Reveal-gating content** — hiding content until scroll hides it from users, search, and assistive tech
- ❌ **Image hover zoom on everything** — template interaction; zoom only when it means "view closer"
- ❌ **Glow pulse / neon hover** on dark UIs — the overused "AI dark mode" look

Full catalog with "why it fails + fix" for each: [ai-tells.md](ai-tells.md).

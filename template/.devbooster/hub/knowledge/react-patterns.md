...
19. [Offload Heavy Computation from the Main Thread](#offload-heavy-computation-from-the-main-thread)
...
_Source: [react.dev — Keeping Components Pure](https://react.dev/learn/keeping-components-pure)_
...
_Source: [react.dev — useMemo](https://react.dev/reference/react/useMemo)_
...
_Source: [react.dev — useState: Avoiding recreating the initial state](https://react.dev/reference/react/useState#avoiding-recreating-the-initial-state)_
...
_Source: [react.dev — Fetching data with Effects](https://react.dev/reference/react/useEffect#fetching-data-with-effects)_
...
For search inputs that need debounce + in-flight request cancellation in layers (timer vs. request), see [Timing Patterns — Prevent stale async responses](./timing-patterns.md).
...
_Source: [react.dev — You Might Not Need an Effect](https://react.dev/learn/you-might-not-need-an-effect)_
...
_Source: [react.dev — useState: Resetting state with a key](https://react.dev/reference/react/useState#resetting-state-with-a-key)_
...
_Source: [react.dev — Specifying reactive dependencies](https://react.dev/reference/react/useEffect#specifying-reactive-dependencies)_
...
_Source: [react.dev — useEffect: Specifying reactive dependencies](https://react.dev/reference/react/useEffect#specifying-reactive-dependencies)_
...
_Source: [react.dev — Updating Objects in State](https://react.dev/learn/updating-objects-in-state)_
...
For parallel fetching, start all requests up front and then decide per group: `Promise.all()` for mandatory data (fail fast, error state) vs `Promise.allSettled()` for optional/enhancement data (degrade gracefully, per-section fallback) — see `hub/skills/nextjs-react-expert/1-async-eliminating-waterfalls.md` (Rule 1.6).

_Sources: [React — Fetching data with Effects](https://react.dev/reference/react/useEffect#fetching-data-with-effects) · [React — You Might Not Need an Effect](https://react.dev/learn/you-might-not-need-an-effect)_
...
_Sources: [React — Suspense](https://react.dev/reference/react/Suspense) · [React — lazy](https://react.dev/reference/react/lazy)_
...
- For CPU-heavy work moved off the main thread with a Web Worker (and its later `useWorker` extraction), see [`react-web-workers.md`](./react-web-workers.md).
...
_Sources: [React — Reusing Logic with Custom Hooks](https://react.dev/learn/reusing-logic-with-custom-hooks) · [React — Rules of Hooks](https://react.dev/reference/rules/rules-of-hooks)_
...
A heavy CPU-bound calculation (large dataset processing, parsing large files, image processing, simulations, compression, cryptographic or data-analysis work) blocks the main thread: rendering, clicks, and scrolling degrade. `async`/`setTimeout` do not move the work off the main thread — only another execution context, such as a Web Worker, does.

Prefer a Web Worker when the task is expensive enough to visibly block the UI and has no DOM dependency; keep cheap operations and network waits on the main thread. Own the Worker instance with `useRef`, create it in an Effect with symmetric cleanup (`terminate()`), and speak to it through a typed message protocol (`type` + `payload`) with progress reporting for long calculations.
...
- Confirm the work has no `document`/page-`window` dependency.
- For the full pattern — lifecycle, protocol, progress, transferables, `useWorker` extraction, and build-tool syntax — see [`react-web-workers.md`](./react-web-workers.md).

_Sources: [MDN — Web Workers API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API) · [React — `useEffect`](https://react.dev/reference/react/useEffect)_

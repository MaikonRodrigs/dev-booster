...
## 🧭 SWIFT SERVER ROUTING (Vapor / Server-Side Swift)

> **PRESERVE THIS SECTION** when rewriting this file during bootstrap.

If this project uses server-side Swift (Vapor, Fluent, `Package.swift` executable targets), load the `swift-backend` skill before backend work:

| Skill           | Bundle                                            | When                                                             |
| --------------- | ------------------------------------------------- | ---------------------------------------------------------------- |
| `swift-backend` | `.devbooster/hub/personas/skill_swift-backend.md` | Vapor routing/models, Fluent ORM, migrations, JWT auth, XCTVapor |

If the project is not Swift-based, ignore this block.

---

## 🌟 PATTERN REFERENCE (THE ELITE STANDARD)

_Use this as your benchmark for technical precision. Even if the technology differs, the depth of detail must match:_

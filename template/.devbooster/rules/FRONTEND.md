...
## 🧭 APPLE NATIVE ROUTING (Swift / SwiftUI / Xcode)

> **PRESERVE THIS SECTION** when rewriting this file during bootstrap — it routes native Apple frontend work to the right skills.

If this project is (or contains) a native Apple app — `.swift` files, `.xcodeproj`/`.xcworkspace`, SwiftUI/AppKit/UIKit — the following skills MUST be loaded before any frontend work:

| Skill        | Bundle                                         | When                                                                      |
| ------------ | ---------------------------------------------- | ------------------------------------------------------------------------- |
| `swift-apps` | `.devbooster/hub/personas/skill_swift-apps.md` | SwiftUI views/state, iOS/macOS/watchOS/tvOS UI, SwiftData, Swift language |
| `xcode-cli`  | `.devbooster/hub/personas/skill_xcode-cli.md`  | Xcode builds, simulators, signing, notarization, shipping                 |

If the project is web-based, ignore this block.

---

## 🧭 TYPESCRIPT POLICY (STRICT_TS)

> **PRESERVE THIS SECTION** when rewriting this file during bootstrap.

- **STRICT_TS:** Zero `any` policy. TypeScript must be structurally perfect at all times — no `any`, no silent suppressions; keep types explicit and complete.

---

## 🌟 PATTERN REFERENCE (THE ELITE STANDARD)

_Use this as your benchmark for UI/UX engineering detail. Maintain this level of rigor regardless of the stack:_
